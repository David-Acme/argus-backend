...
struct ProductivityHandleResult
{
  bool ok = false;
...
struct ProductivityExecInput
{
  sqlite3* db = nullptr;
  std::string sql;
};

struct ProductivityChecksumInput
{
  sqlite3* db = nullptr;
  std::string schema;
  std::string table;
...
struct ProductivityChecksumResult
{
  bool ok = false;
  int64_t rows = 0;
  std::string checksum;
...
struct ProductivityColumnShapeInput
{
  sqlite3* db = nullptr;
  std::string schema;
  std::string table;
...
ProductivityResult execStatement(const ProductivityExecInput& input)
{
...
ProductivityHandleResult openHandle(const std::string& path, int flags)
{
  ProductivityHandleResult result;
  sqlite3* raw = nullptr;
...
columnNames(const ProductivityColumnShapeInput& input, std::string& error)
{
...
bool hasAllProductivityTables(sqlite3* db, std::string& missing)
{
  for (const auto& table : kProductivityTables) {
    SqliteStmt stmt;
...
bool copyProductivityTables(sqlite3* db, std::string& error)
{
...
ProductivityResult validateDistinctPaths(const std::string& sourcePath,
...
ProductivityResult validateSource(const std::string& sourcePath)
{
  ProductivityResult result;
  std::error_code ec;
  ...
  std::string missing;
  if (!hasAllProductivityTables(source.db.get(), missing)) {
    result.error = "source database is missing productivity table: " + missing;
    return result;
  }
  result.ok = true;
  return result;
...
ProductivityMigrationReport verifyForeignKeyIntegrity(sqlite3* target)
{
  ProductivityMigrationReport report;
  SqliteStmt stmt;
    ...
    const ProductivityVerificationInput& input)
{
  ...
  for (const auto& table : kProductivityTables) {
    ProductivityTableReport entry{.table = table,
                                  .sourceRows = 0,
                                  .targetRows = 0,
                                  .sourceChecksum = std::string(),
    ...
    const ProductivityMigrationOptions& options)
{
  ...
  if (std::filesystem::is_regular_file(options.targetPath, targetEc)) {
    ...
    std::string missing;
    if (!hasAllProductivityTables(existing.db.get(), missing)) {
      report.error = "target productivity.db exists but is not schema-current "
                     "(missing productivity table: " + missing
    ...
    for (const auto& table : kProductivityTables) {
      ...
      if (*existingColumns != *referenceColumns) {
        report.error = "target productivity.db exists but is not "
  ...
  if (!copyProductivityTables(target.db.get(), copyError)) {
  ...
  report = verifyProductivityTables({.target = target.db.get()});
