...
void NatsNotificationChangeSink::emitUser(int64_t userId,
                                          const SocketEmitDto& body) const
{
  bus_->publish(nats_subject::kNotificationChange,
                json_util::toString(sync_change::userEmitPayload(body,
                                                                 {userId})));
}

void NatsNotificationChangeSink::emitUsers(
    ...
    const UserAuditInput& input) const
{
  ...
  if (!bus_->publish(nats_subject::kNotificationChange,
                     json_util::toString(event.toJson())))
    LOG_WARN << "Notification audit funnel: publish failed for record "
             << input.recordId;
  co_return;
}