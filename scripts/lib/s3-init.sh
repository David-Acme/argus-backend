...
create_output=$(/usr/bin/rc admin service-account create argus "$app_access_key" \
  "$app_secret_key" --name argus-backend \
  --description "Argus private object storage" \
  --policy-json "$policy_json" 2>&1) || {
...
if ! /usr/bin/rc admin service-account info argus "$app_access_key" >/dev/null
then
  echo "RustFS application service account could not be verified" >&2
  exit 1
fi
