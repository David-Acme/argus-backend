...
int main()
{
  ...
  for (int y = 0; y < img.rows; ++y) {
    ...
    for (int x = 0; x < img.cols; ++x) {
      ...
      row[x * 3 + 2] = static_cast<unsigned char>(((x + y) * 127) / (img.cols + img.rows));
    }
  }
  cv::rectangle(img, cv::Rect(200, 140, 320, 360), cv::Scalar(240, 240, 240), -1);
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    const auto caption = VisionService::describeMat(img);
  ...
  t0 = std::chrono::steady_clock::now();
  const auto answer = VisionService::describeMat(
      img, "Is there a person in this image? Answer yes or no.", 12);
  const double ms = std::chrono::duration<double, std::milli>(
      std::chrono::steady_clock::now() - t0).count();
