...
struct NotificationHandleResult
{
  bool ok = false;
...
struct NotificationExecInput
{
  sqlite3* db = nullptr;
  std::string sql;
};

struct NotificationChecksumInput
{
  sqlite3* db = nullptr;
  std::string schema;
  std::string table;
...
struct NotificationChecksumResult
{
  bool ok = false;
  int64_t rows = 0;
  std::string checksum;
...
struct NotificationColumnShapeInput
{
  sqlite3* db = nullptr;
  std::string schema;
  std::string table;
...
NotificationResult execStatement(const NotificationExecInput& input)
{
...
NotificationHandleResult openHandle(const std::string& path, int flags)
{
...
NotificationChecksumResult tableChecksum(
    const NotificationChecksumInput& input)
{
  NotificationChecksumResult result;
...
columnNames(const NotificationColumnShapeInput& input, std::string& error)
{
...
bool hasAllNotificationTables(sqlite3* db, std::string& missing)
{
  for (const auto& table : kNotificationTables) {
    SqliteStmt stmt;
...
bool copyNotificationTables(sqlite3* db, std::string& error)
{
...
NotificationResult validateDistinctPaths(const std::string& sourcePath,
...
NotificationResult validateSource(const std::string& sourcePath)
{
  NotificationResult result;
  std::error_code ec;
  ...
  std::string missing;
  if (!hasAllNotificationTables(source.db.get(), missing)) {
    result.error = "source database is missing notification table: " + missing;
    return result;
  }
  result.ok = true;
  return result;
...
NotificationHandleResult schemaReference(const std::string& schemaPath)
{
  ...
  const auto schema = applyNotificationSchema(
      {.db = reference.db.get(), .schemaPath = schemaPath});
  if (!schema.ok) {
    reference.ok = false;
...
NotificationMigrationReport verifyForeignKeyIntegrity(sqlite3* target)
{
  NotificationMigrationReport report;
  SqliteStmt stmt;
  ...
  while (stmt.step() == SQLITE_ROW) {
    ...
    report.error = "foreign_key_check reported violations after the copy ("
                   + std::string(stmt.columnText(0)) + " -> " + parent + ")";
    return report;
  }
  report.ok = true;
  return report;
    ...
    const NotificationVerificationInput& input)
{
  ...
  for (const auto& table : kNotificationTables) {
    NotificationTableReport entry{.table = table,
                                  .sourceRows = 0,
                                  .targetRows = 0,
                                  .sourceChecksum = std::string(),
    ...
    const NotificationMigrationOptions& options)
{
  ...
  if (std::filesystem::is_regular_file(options.targetPath, targetEc)) {
    ...
    std::string missing;
    if (!hasAllNotificationTables(existing.db.get(), missing)) {
      report.error = "target notification.db exists but is not schema-current "
                     "(missing notification table: " + missing
    ...
    for (const auto& table : kNotificationTables) {
      ...
      if (*existingColumns != *referenceColumns) {
        report.error = "target notification.db exists but is not "
                       "schema-current (column shape of " + table
  ...
  if (!copyNotificationTables(target.db.get(), copyError)) {
  ...
  report = verifyNotificationTables({.target = target.db.get()});
